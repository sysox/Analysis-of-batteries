...
def draw_hist(freqs, expected_freq, axis, text='Histogram'):
    ...
    axis2.set_ylabel('Relative frequencies', labelpad=0)
    axis.set_xlabel('Histogram bins', labelpad=0)
    axis.set_ylabel('Absolute frequencies', labelpad=0)
    axis.hlines(y=expected_freq, xmin=1, xmax=len(freqs), colors='b', linestyles='dashed', lw = 1)

def draw_ECDF(values, axis, label="ECDF"):
    ecdf = ECDF(values)
    ecdf.KS()
    ecdf.plot(domain=(0,1), ax=axis, label=label)
...
def draw_KS_relative(values, axis, text):
    ...
    axis.set_xlim(xmin=0, xmax=n+0.5)
    axis.set_xticks([1,index_min, index_max, n])
    axis.set_yticks(([0, ymin, ymax]))
    axis.scatter(x, y, s=2, color = color)
    axis.hlines(y=0, xmin=1, xmax=n+1, colors='b', linestyles='dashed', lw=1)
    axis.set_xlabel('Value index', labelpad=0)
    axis.set_ylabel('Relative $D_n$', labelpad=0)
    # axis2 = axis.twinx()
    # axis2.set_xticks([])
    # axisx2 = axis.twiny()
...
def draw_tails(tails, axis, text):
    y = np.array([round(tail,2) for tail in tails])
    ...
    axis.set_ylim(ymin=ymin*0.9, ymax=ymax*1.1)
    ...
    axis.set_xlabel('Tail id', labelpad=0)
    axis.set_ylabel('Tail ratio', labelpad=0)
    axis2.set_ylabel('Reciprocal ratio ', labelpad=0)
...
if __name__ == '__main__':
    ...
    # bar histogram
    draw_hist(freqs=[1, 2, 3, 4, 5], expected_freq=3, axis=axs[0][0], text="Histogram")

    # ECDF
    pvals = uniform_random(10)
    draw_ECDF(values=pvals, axis=axs[0][1], label="$D_n$ and ECDF")
